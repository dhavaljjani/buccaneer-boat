...
public class Container extends Item {
	...
	@Override
	public void doUse() {
		if(this.getName().equals("cannon")) {
			if(this.getItems().size() == 0) {
				World.print("This cannon is unloaded and lacks ammunition, maybe find something to put in it.... \n\n");
			} else if(this.getItems().size() == 1 && this.getItems().get(0).getName().equals("cannonball")) {
				World.print("You grab the ramrod and ram it down the cannon's barrel, and light the fuse"
						+ " and wait for something to happen, until BANG! Due to your haste (and incompetancy) with the cannon, you"
						+ " the cannon explodes into the mast at an awkward trajectory, causing a mini explosion. \n\n");
				World.print("GAME OVER ; You killed yourself and six other crewmen with your boneheaded decision to use the cannonball for some reason.");
				System.exit(0);
			}
		}
	}
	...
	public void doExamine() {
		if((Container)getWorld().getPlayer().getCurrentRoom().getItem(this.getName()) instanceof Person) {
			if(this.getName().equals("old_man")) {
				if(getWorld().getPlayer().hasItem("silver_tooth")) {
					World.print("Your neighboring inmate is an old man, around 80. He peers at you angrily through the prison bars.\n\n");
					return;
				}
				World.print("Your neighboring inmate is an old man, around 80, with a silver tooth. He peers at you through the prison bars.\n\n");
				return;
			}
		}
